...
const BrowseProgramsPage = () => {
  ...
  const [pendingHomeProgram, setPendingHomeProgram] = useState(null);
  ...
  const handleGetProgram = async (program) => {
    ...
    // Home programs need equipment selection first
    if (program.type === 'home') {
      const savedEquipment = localStorage.getItem('fitstart_equipment');
      if (!savedEquipment) {
        setPendingHomeProgram(program);
        setShowEquipmentModal(true);
        return;
      }
      await enrollProgram(program);
      return;
    }
  ...
  const handleEquipmentConfirm = async (equipment) => {
    setShowEquipmentModal(false);
    if (pendingHomeProgram) {
      await enrollProgram(pendingHomeProgram);
      setPendingHomeProgram(null);
    }
  };
    ...
    <div className="min-h-screen bg-[#0f0f0f]" data-testid="browse-programs-page">
      {showEquipmentModal && (
        <EquipmentModal
          ...
          onClose={() => { setShowEquipmentModal(false); setPendingHomeProgram(null); }}
        />
      )}
      {/* Navbar */}
